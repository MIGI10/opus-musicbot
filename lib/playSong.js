...
async function play(client, song, queue, errorOccurred, guild) {
    ...
    queue.player.once(voice.AudioPlayerStatus.Idle, () => {
        ...
        if (queue.loop) {
            ...
            return play(client, queue.songs[0], queue, false, guild);
            ...
            if (queue.songs.length >= 1) {
                ...
                if (queue.shuffle) {
                    ...
                    var songToMove = queue.songs[randomSong];
                    queue.songs.splice(randomSong, 1);
                    queue.songs.splice(0, 0, songToMove);
                ...
                queue.inactivity = setTimeout(() => {
                    ...
                    if (!queue.playing && !queue.songs[0]) {
                        ...
                        if (queue.connection._state.status != 'destroyed') {
